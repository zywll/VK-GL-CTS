        ...
        for (int levelNdx = 0; levelNdx < getNumLevels(); levelNdx++)
        {
            for (int layerNdx = 0; layerNdx < getArraySize(); layerNdx++)
            {
                ...
                const VkBufferImageCopy layerRegion = {
                    ...
                    (uint32_t)getNextMultiple(blockPixelSize.x(), level.getWidth()),  // uint32_t bufferRowLength;
                    (uint32_t)getNextMultiple(blockPixelSize.y(), level.getHeight()), // uint32_t bufferImageHeight;
