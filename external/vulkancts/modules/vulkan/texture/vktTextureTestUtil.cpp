                        ...
                        glu::Precision fragOutputPrecision, bool unnormal)
{
    ...
    for (std::vector<Program>::const_iterator programIt = programs.begin(); programIt != programs.end(); ++programIt)
    {
        ...
        case PROGRAM_2D_FLOAT:
            sampler = "sampler2D";
            lookup  = texture + "(u_sampler, texCoord" + lod + ")";
            break;
        case PROGRAM_2D_INT:
            sampler = "isampler2D";
            lookup  = "vec4(" + texture + "(u_sampler, texCoord" + lod + "))";
            break;
        case PROGRAM_2D_UINT:
            sampler = "usampler2D";
            lookup  = "vec4(" + texture + "(u_sampler, texCoord" + lod + "))";
        ...
        case PROGRAM_2D_FETCH_LOD:
            sampler = "sampler2D";
            lookup  = "texelFetch(u_sampler, ivec2(texCoord * vec2(64.f), 3)";
            break;
        case PROGRAM_2D_SHADOW:
            sampler = "sampler2DShadow";
            lookup  = "vec4(" + texture + "(u_sampler, vec3(texCoord, u_ref)" + lod + "), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_2D_FLOAT_BIAS:
            sampler = "sampler2D";
            lookup  = "texture(u_sampler, texCoord, u_bias)";
            break;
        case PROGRAM_2D_INT_BIAS:
            sampler = "isampler2D";
            lookup  = "vec4(texture(u_sampler, texCoord, u_bias))";
            break;
        case PROGRAM_2D_UINT_BIAS:
            sampler = "usampler2D";
            lookup  = "vec4(texture(u_sampler, texCoord, u_bias))";
        ...
        case PROGRAM_2D_SHADOW_BIAS:
            sampler = "sampler2DShadow";
            lookup  = "vec4(texture(u_sampler, vec3(texCoord, u_ref), u_bias), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_1D_FLOAT:
            sampler = "sampler1D";
            lookup  = texture + "(u_sampler, texCoord" + lod + ")";
        ...
        case PROGRAM_1D_SHADOW:
            ...
            lookup  = "vec4(" + texture + "(u_sampler, vec3(texCoord, 0.0, u_ref)" + lod + "), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_1D_FLOAT_BIAS:
            sampler = "sampler1D";
            lookup  = "texture(u_sampler, texCoord, u_bias)";
        ...
        case PROGRAM_1D_SHADOW_BIAS:
            sampler = "sampler1DShadow";
            lookup  = "vec4(texture(u_sampler, vec3(texCoord, 0.0, u_ref), u_bias), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_CUBE_FLOAT:
            sampler = "samplerCube";
            lookup  = "texture(u_sampler, texCoord)";
            break;
        case PROGRAM_CUBE_INT:
            sampler = "isamplerCube";
            lookup  = "vec4(texture(u_sampler, texCoord))";
            break;
        case PROGRAM_CUBE_UINT:
        ...
        case PROGRAM_CUBE_SHADOW:
            ...
            lookup  = "vec4(texture(u_sampler, vec4(texCoord, u_ref)), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_CUBE_FLOAT_BIAS:
            sampler = "samplerCube";
            lookup  = "texture(u_sampler, texCoord, u_bias)";
        ...
        case PROGRAM_CUBE_SHADOW_BIAS:
            sampler = "samplerCubeShadow";
            lookup  = "vec4(texture(u_sampler, vec4(texCoord, u_ref), u_bias), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_2D_ARRAY_FLOAT:
        ...
        case PROGRAM_CUBE_ARRAY_SHADOW:
            ...
            lookup  = "vec4(texture(u_sampler, texCoord, u_ref), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_1D_ARRAY_FLOAT:
        ...
        case PROGRAM_1D_ARRAY_SHADOW:
            ...
            lookup  = "vec4(texture(u_sampler, vec3(texCoord, u_ref)), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_BUFFER_FLOAT:
            sampler = "samplerBuffer";
            lookup  = "texelFetch(u_sampler, int(texCoord))";
            break;
        case PROGRAM_BUFFER_INT:
            sampler = "isamplerBuffer";
            lookup  = "vec4(texelFetch(u_sampler, int(texCoord)))";
            break;
        case PROGRAM_BUFFER_UINT:
...
void TextureBinding::updateTextureData(const TestTextureSp &textureData, const TextureBinding::Type textureType)
{
    ...
    const uint32_t queueFamilyIndices[] = {m_context.getUniversalQueueFamilyIndex(),
                                           m_context.getSparseQueueFamilyIndex()};
    m_type                              = textureType;
    m_textureData                       = textureData;
    ...
    const VkSharingMode sharingMode =
        (sparse && m_context.getUniversalQueueFamilyIndex() != m_context.getSparseQueueFamilyIndex()) ?
    ...
    if (sparse)
    {
        pipeline::uploadTestTextureSparse(
            vkd, m_device, m_context.getPhysicalDevice(), m_context.getInstanceInterface(), imageParams, queue,
            queueFamilyIndex, m_context.getSparseQueue(), m_allocator, m_allocations, *m_textureData, *m_textureImage);
    }
    else
    {
...
const uint16_t TextureRenderer::s_vertexIndices[6]          = {0, 1, 2, 2, 1, 3};
...
void TextureRenderer::add2DTexture(const TestTexture2DSp &texture, const vk::VkImageAspectFlags &aspectMask,
                                   TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(
        TextureBindingSp(new TextureBinding(m_context, getDevice(), *m_allocator, texture, TextureBinding::TYPE_2D,
                                            aspectMask, backingMode, m_componentMapping)));
}

void TextureRenderer::addCubeTexture(const TestTextureCubeSp &texture, const vk::VkImageAspectFlags &aspectMask,
                                     TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(TextureBindingSp(new TextureBinding(m_context, getDevice(), *m_allocator, texture,
                                                                    ...
                                                                    backingMode, m_componentMapping)));
}

void TextureRenderer::add2DArrayTexture(const TestTexture2DArraySp &texture, const vk::VkImageAspectFlags &aspectMask,
                                        TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(TextureBindingSp(new TextureBinding(m_context, getDevice(), *m_allocator, texture,
...
void TextureRenderer::add3DTexture(const TestTexture3DSp &texture, const vk::VkImageAspectFlags &aspectMask,
                                   TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(
...
void TextureRenderer::add1DTexture(const TestTexture1DSp &texture, const vk::VkImageAspectFlags &aspectMask,
                                   TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(
...
void TextureRenderer::add1DArrayTexture(const TestTexture1DArraySp &texture, const vk::VkImageAspectFlags &aspectMask,
                                        TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(TextureBindingSp(new TextureBinding(m_context, getDevice(), *m_allocator, texture,
...
void TextureRenderer::addCubeArrayTexture(const TestTextureCubeArraySp &texture,
                                          const vk::VkImageAspectFlags &aspectMask,
                                          TextureBinding::ImageBackingMode backingMode)
{
    m_textureBindings.push_back(TextureBindingSp(new TextureBinding(m_context, getDevice(), *m_allocator, texture,
...
uint32_t TextureRenderer::getRenderWidth(void) const
{
    return m_renderWidth;
}

uint32_t TextureRenderer::getRenderHeight(void) const
{
    return m_renderHeight;
}

Move<VkDescriptorSet> TextureRenderer::makeDescriptorSet(const VkDescriptorPool descriptorPool,
...
void TextureRenderer::addImageTransitionBarrier(VkCommandBuffer commandBuffer, VkImage image,
                                 ...
                                 const float maxAnisotropy)
{
    ...
    // Create Vertex Buffer
    {
        ...
        vertexBufferMemory = m_allocator->allocate(getBufferMemoryRequirements(vkd, vkDevice, *vertexBuffer),
                                                   MemoryRequirement::HostVisible);
        ...
        deMemcpy(reinterpret_cast<uint8_t *>(vertexBufferMemory->getHostPtr()) + positionDataSize, texCoord,
                 textureCoordDataSize);
        flushMappedMemoryRange(vkd, vkDevice, vertexBufferMemory->getMemory(), vertexBufferMemory->getOffset(),
                               VK_WHOLE_SIZE);
    }
    ...
    // Copy Image
    {
        ...
        addImageTransitionBarrier(
            *commandBuffer, m_multisampling ? *m_resolvedImage : *m_image,
            VK_PIPELINE_STAGE_TRANSFER_BIT,                // VkPipelineStageFlags        srcStageMask
